...
`,Io=[{mode:"replace",icon:"mdi:playlist-remove",label:h("search.replace")},{mode:"next",icon:"mdi:playlist-play",label:h("search.play_next")},{mode:"replace_next",icon:"mdi:playlist-music",label:h("search.replace_play")},{mode:"add",icon:"mdi:playlist-plus",label:h("search.add_queue")},{mode:"add_to_playlist",icon:"mdi:plus",label:h("search.add_to_playlist")}],Mt=["artist","album","track","playlist","radio","podcast","audiobook"];function bs(i){return i&&(i.media_class==="track"||i.media_content_type==="track")}const Ne=(i,{cap:e,floor:t}={})=>{const a=Number(i);if(!Number.isFinite(a)||a<=0)return;let s=a;return typeof t=="number"&&(s=Math.max(t,s)),typeof e=="number"&&(s=Math.min(e,s)),s},xs=3e4;let Ft=null,Ki=0;async function oi(i){const e=Date.now();if(Ft&&e-Ki<xs)return Ft;try{return Ft=(await i.callApi("GET","config/config_entries/entry")).find(t=>t.domain==="music_assistant"&&t.state==="loaded")?.entry_id||null,Ki=e,Ft}catch(t){return console.error("yamp: Failed to resolve Music Assistant config entry",t),Ft=null,Ki=e,null}}let Dt=null,Xi=0;async function Ji(i){const e=Date.now();if(Dt&&e-Xi<xs)return Dt;try{return Dt=(await i.callApi("GET","config/config_entries/entry")).find(t=>t.domain==="mass_queue"&&t.state==="loaded")?.entry_id||null,Xi=e,Dt}catch(t){return console.error("yamp: Failed to resolve mass_queue config entry",t),Dt=null,Xi=e,null}}function ct(i){return i?{title:i.name,media_content_id:i.uri,media_content_type:i.media_type,media_class:i.media_type,item_id:i.item_id,thumbnail:i.image,...i.artists&&{artist:i.artists.map(e=>e.name).join(", ")},...i.album&&{album:i.album.name},is_browsable:i.media_type==="artist"||i.media_type==="album"||i.media_type==="playlist",is_editable:i.is_editable===!0}:null}function li({item:i,onPlay:e,onOptionsToggle:t,upcomingFilterActive:a=!1,isMusicAssistant:s=!1,massQueueAvailable:r=!1,searchView:n="list",isInline:o=!1,onMoveUp:l,onMoveDown:c,onMoveNext:u,onRemove:d}){const p=!!(a&&i.queue_item_id&&s&&r),_=o?"entity-options-search-buttons":n==="card"?"card-overlay-buttons":"search-sheet-buttons",f=o?"entity-options-search-play":n==="card"?"search-sheet-play icon-only":"search-sheet-play",g=o?"entity-options-search-queue":n==="card"?"search-sheet-queue icon-only":"search-sheet-queue";return m`
  ...
  `}function Co({open:i,query:e,onQueryInput:t,onSearch:a,onClose:s,loading:r,results:n,onPlay:o,onQueue:l,error:c,showQueueSuccess:u,matchTheme:d=!1,upcomingFilterActive:p=!1,disableAutofocus:_=!1,activeSearchRowMenuId:f,successSearchRowMenuId:g,successSearchRowType:b,onOptionsToggle:k,onPlayOption:x,onResultClick:T,searchView:E="list",searchCardColumns:H=4,massQueueAvailable:j=!1,onMoveUp:F,onMoveDown:z,onMoveNext:R,onRemove:B}){return i?m`
    <div class="search-sheet" data-match-theme="${d}" data-card-view="${E==="card"}">
      ...
      <div class="search-sheet-results ${E==="card"?"search-results-card-view":"list-view"}">
        ${(n||[]).length===0&&!r?m`<div class="search-sheet-empty">${h("common.no_results")}</div>`:(n||[]).map(A=>{const L=tt(A.media_content_id),q=A.is_browsable&&(A.media_class!=="playlist"||j);return m`
                <div class="search-sheet-result ${E==="card"?"search-result-card":""}">
                  <div class="search-sheet-thumb-container" 
                    ...
                    `}
                    ${E==="card"?li({item:A,onPlay:o,onOptionsToggle:k,upcomingFilterActive:p,isMusicAssistant:L,massQueueAvailable:j,searchView:"card",onMoveUp:F,onMoveDown:z,onMoveNext:R,onRemove:B}):y}
                  </div>
                  ...
                  ${E!=="card"?li({item:A,onPlay:o,onOptionsToggle:k,upcomingFilterActive:p,isMusicAssistant:L,massQueueAvailable:j,searchView:"list",isInline:!0,onMoveUp:F,onMoveDown:z,onMoveNext:R,onRemove:B}):y}
                  
                  ${ws({item:A,activeSearchRowMenuId:f,successSearchRowMenuId:g,successSearchRowType:b,onPlayOption:x,onOptionsToggle:k,searchView:E,isQueueItem:L&&A.queue_item_id&&p&&j,massQueueAvailable:j,onMoveUp:F,onMoveDown:z,onMoveNext:R,onRemove:B})}
                </div>
              `})}
      </div>
    </div>
...
{% endif %}</pre>
    ...
    `}_renderGroupingMenuOption(){if(this.entityIds.length<=1)return y;const e=this.entityIds.reduce((n,o,l)=>{const c=this._getGroupingEntityId(l),u=this.hass.states[c];return n+(this._isGroupCapable(u)?1:0)},0),t=this._getGroupingEntityId(this._selectedIndex),a=this.hass.states[t],s=this.currentEntityId,r=this._getGroupKey(s)!==s;return e>1&&this._isGroupCapable(a)&&!r?m`
      ...
      <div class="group-list-scroll">
        ...
        `:f.map(g=>{const b=g.id,k=g.groupId,x=(Array.isArray(s?.attributes?.group_members)?s.attributes.group_members:[]).includes(k),T=this.getChipName(b),E=g.isBusy,H=g.busyLabel,j=this.entityIds.indexOf(b),F=this._getVolumeEntityForGrouping(j)||k,z=this.hass.states[F],R=F?.startsWith&&F.startsWith("remote."),B=Number(z?.attributes?.volume_level||0),A=b===e,L=!A;let q=h(r?A?"card.grouping.master":x?"card.grouping.joined":"card.grouping.available":b===l?"card.grouping.current":"card.grouping.available");return E&&(q=H||"Unavailable"),m`
                ...
                ${R?m`
                  ...
                  `:m`
                    <input
                      ...
                      step="0.01"
                      .value=${B}
                      @change=${Z=>this._onGroupVolumeChange(b,F,Z)}
                      title="Volume"
