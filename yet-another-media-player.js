          ...
          <div class="entity-options-sheet${d||f?" chips-mode":""} entity-options-sheet-opening" 
            ...
            ${d||f?m`
                ...
                <div class="chip-row entity-options-chips-strip">
                  ${gs({groupedSortedEntityIds:this.groupedSortedEntityIds,entityIds:this.entityIds,selectedEntityId:this.currentEntityId,pinnedIndex:this._pinnedIndex,holdToPin:this._holdToPin,getChipName:g=>this.getChipName(g),getActualGroupMaster:g=>this._getActualGroupMaster(g),getIsChipPlaying:(g,S)=>{const j=this._findEntityObjByAnyId(g)?.entity_id||g,P=this.entityIds.indexOf(j);if(P<0)return!1;const $=this._getEntityForPurpose(P,"playback_control"),ce=this.hass?.states?.[$];return this._isEntityPlaying(ce)},getChipArt:g=>{const S=this._findEntityObjByAnyId(g)?.entity_id||g,j=this.entityIds.indexOf(S);if(j<0)return null;const P=this._getEntityForPurpose(j,"playback_control"),$=this.hass?.states?.[P],ce=this.hass?.states?.[S],Ni=this._getArtworkUrl($),qi=this._getArtworkUrl(ce);return Ni||qi},getIsMaActive:g=>{const S=this._findEntityObjByAnyId(g)?.entity_id||g,j=this.entityIds.indexOf(S);if(j<0)return!1;const P=this.entityObjs[j];if(!P?.music_assistant_entity)return!1;const $=this._getEntityForPurpose(j,"playback_control"),ce=this.hass?.states?.[$];return $===this._resolveEntity(P.music_assistant_entity,P.entity_id,j)&&this._isEntityPlaying(ce)},isIdle:this._isIdle,hass:this.hass,artworkHostname:this.config?.artwork_hostname||"",mediaArtworkOverrides:this.config?.media_artwork_overrides||[],fallbackArtwork:this.config?.fallback_artwork||null,onChipClick:g=>this._onChipClick(g),onIconClick:(g,S)=>{const j=this.entityIds[g],P=this.groupedSortedEntityIds.find($=>$.includes(j));P&&P.length>1&&(this._selectedIndex=g,this._showEntityOptions=!0,this._showGrouping=!0,this.requestUpdate())},onPinClick:(g,S)=>{S.stopPropagation(),this._onPinClick(S)},onPointerDown:(g,S)=>this._handleChipPointerDown(g,S),onPointerMove:(g,S)=>this._handleChipPointerMove(g,S),onPointerUp:(g,S)=>this._handleChipPointerUp(g,S)})}
                </div>
              </div>
            `:b}
            ...
            <div class="entity-options-search-results ${this.config.search_view==="card"?"search-results-card-view":"list-view"}" 
              ...
              ${(()=>{this._searchMediaClassFilter;const g=this._getDisplaySearchResults(),S=Math.max(15,this._searchTotalRows||g.length),j=[...g,...Array.from({length:Math.max(0,S-g.length)},()=>null)];return this._searchAttempted&&g.length===0&&!this._searchLoading?m`<div class="entity-options-search-empty" style="color: white;">No results.</div>`:j.map(P=>P?m`
                            ...
                            <div class="entity-options-search-result ${this.config.search_view==="card"?"search-result-card":""} ${P._justMoved?"just-moved":""} ${P.media_content_id!=null&&this._activeSearchRowMenuId===P.media_content_id?"menu-active":""}">
                               <div class="search-sheet-thumb-container"
                                ...
                                `}
                                ${this.config.search_view==="card"?oi({item:P,onPlay:$=>this._playMediaFromSearch($),onOptionsToggle:$=>{this._activeSearchRowMenuId=$?.media_content_id||null,this.requestUpdate()},upcomingFilterActive:!!this._upcomingFilterActive,isMusicAssistant:this._isMusicAssistantEntity(),massQueueAvailable:this._massQueueAvailable,searchView:"card"}):b}
                               </div>
                               <div class="search-sheet-info">
                                <span class="${this._isClickableSearchResult(P)?"clickable-search-result":""} ${this.config.search_view==="card"?"search-sheet-title":""}"
                                      @touchstart=${$=>this._handleSearchResultTouch(P,$)}
                                      @click=${()=>this._handleSearchResultClick(P)}
                                      title=${this._getSearchResultClickTitle(P)}>
                                  ${P.title}
                                </span>
                                 <span class="search-sheet-subtitle">
                                  ${(()=>{const $=this._searchMediaClassFilter==="track"||this._searchMediaClassFilter==="album",ce=!!this._recentlyPlayedFilterActive,Ni=!!this._upcomingFilterActive,qi=!!this._recommendationsFilterActive;return($||ce||Ni||qi)&&P.artist?P.artist:P.media_class?P.media_class.charAt(0).toUpperCase()+P.media_class.slice(1):""})()}
                                </span>
                                ${this.config.search_view==="card"?m`
                                  <div class="card-menu-button" @click=${$=>{$.preventDefault(),$.stopPropagation(),this._activeSearchRowMenuId=P.media_content_id,this.requestUpdate()}}>
                                    <ha-icon icon="mdi:dots-vertical"></ha-icon>
                                  </div>
                                `:b}
                              </div>
                              ${this.config.search_view!=="card"?oi({item:P,onPlay:$=>this._playMediaFromSearch($),onOptionsToggle:$=>{this._activeSearchRowMenuId=$?.media_content_id||null,this.requestUpdate()},upcomingFilterActive:!!this._upcomingFilterActive,isMusicAssistant:this._isMusicAssistantEntity(),massQueueAvailable:this._massQueueAvailable,searchView:"list",isInline:!0,onMoveUp:$=>this._moveQueueItemUp($.queue_item_id),onMoveDown:$=>this._moveQueueItemDown($.queue_item_id),onMoveNext:$=>this._moveQueueItemNext($.queue_item_id),onRemove:$=>this._removeQueueItem($.queue_item_id)}):b}
                                ...
                                ${bs({item:P,activeSearchRowMenuId:this._activeSearchRowMenuId,successSearchRowMenuId:this._successSearchRowMenuId,onPlayOption:($,ce)=>this._performSearchOptionAction($,ce),onOptionsToggle:$=>{this._activeSearchRowMenuId=$?.media_content_id||null,this.requestUpdate()},searchView:this.config.search_view,isQueueItem:this._isMusicAssistantEntity()&&P.queue_item_id&&!!this._upcomingFilterActive&&this._massQueueAvailable,onMoveUp:$=>this._moveQueueItemUp($.queue_item_id),onMoveDown:$=>this._moveQueueItemDown($.queue_item_id),onMoveNext:$=>this._moveQueueItemNext($.queue_item_id),onRemove:$=>this._removeQueueItem($.queue_item_id)})}
                            </div>
                          `:m`
              ...
              `:this._showGrouping?this._renderGroupingSheet():m`
                ...
                <div class="floating-source-index">
                  ${_e.map((g,S)=>{const j=Se.has(g),P=this._hoveredSourceLetterIndex;let $="";if(j&&P!==null&&P!==void 0){const ce=Math.abs(P-S);ce===0?$="max":ce===1?$="large":ce===2&&($="med")}return m`
                      <button
...
`}
            ...
            ${Ga?m`
              <div class="persistent-media-controls" @click=${g=>g.stopPropagation()}>
                ...
                ${(()=>{const g=this._selectedIndex,S=this._getVolumeEntity(g);if(!S)return b;const j=S.startsWith&&S.startsWith("remote."),P=this.currentVolumeStateObj,$=Number(P?.attributes?.volume_level??0),ce=j?null:`${Math.round(($||0)*100)}%`;return this.config.volume_mode==="hidden"?b:m`
