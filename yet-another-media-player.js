    ...
    <div class="entity-options-overlay entity-options-overlay-opening" @click=${e}>
      <div class="entity-options-container entity-options-sheet-opening" @click=${r=>r.stopPropagation()}>
        <div class="entity-options-sheet">
          ...
          ${Ro.filter(r=>r.mode==="add_to_playlist"?$r(i)&&a:!0).map(r=>_`
            ...
            <div class="card-lower-content-container" style="${Ja?`min-height:${Ja}px;`:""}">
              ...
              <div class="card-lower-content${Q?" collapsed transitioning":" transitioning"}${Q&&le?" has-artwork":""}" style="${Ye?Q?`min-height: ${this._collapsedBaselineHeight||220}px;`:"min-height: 350px;":""}">
                ...
                ${Li||!Q?_`
                  ...
                  >
                    ${si&&le?_`
                      <div style="position: absolute; ${Rn?"top: 20px; right: 0; bottom: 0; left: 0;":"inset: 0;"} display: flex; align-items: center; justify-content: center; pointer-events: none;">
                        <img 
                          class="inset-artwork"
                          src="${le}" 
                    ...
                    ${this._lyricsActive?_`
                      <yamp-lyrics-view
                        ...
                        @seek-lyric=${b=>this._onLyricSeek(b.detail.time)}
                      ></yamp-lyrics-view>
                    `:v}
                  </div>
                `:v}
        ...
        <div class="entity-options-container entity-options-container-opening">
          <div class="entity-options-sheet${u||f?" chips-mode":""} entity-options-sheet-opening" 
            ...
            <div class="entity-options-search-results ${this.config.search_view==="card"||this.config.search_view==="card_minimal"?"search-results-card-view":"list-view"}" 
              ...
              ${(()=>{this._searchMediaClassFilter;const b=this._getDisplaySearchResults(),O=this.config.search_view==="card"||this.config.search_view==="card_minimal",G=this.config.search_view==="card_minimal",tt=Math.max(15,this._searchTotalRows||b.length),qe=[...b,...Array.from({length:Math.max(0,tt-b.length)},()=>null)];return this._searchAttempted&&b.length===0&&!this._searchLoading?_`<div class="entity-options-search-empty">${d("common.no_results")}</div>`:qe.map(C=>C?_`
                            ...
                            <div class="entity-options-search-result ${O?"search-result-card":""} ${G?"minimal":""} ${C._justMoved?"just-moved":""} ${C.media_content_id!=null&&this._activeSearchRowMenuId===C.media_content_id?"menu-active":""}">
                               <div class="search-sheet-thumb-container"
                                ...
                                `:_`
                                 ...
                                 `}
                                 ${(()=>{const ue=this._isSelectionFlow;return O?Tr({item:C,onPlay:R=>this._playMediaFromSearch(R),onOptionsToggle:R=>{this._activeSearchRowMenuId=R?.media_content_id||null,this.requestUpdate()},upcomingFilterActive:!!this._upcomingFilterActive,isMusicAssistant:this._isMusicAssistantEntity(),massQueueAvailable:this._massQueueAvailable,searchView:"card",minimal:G,hideActions:ue}):v})()}
                               </div>
                                ${G?v:_`
                                <div class="search-sheet-info">
                                 <span class="${this._isClickableSearchResult(C)?"clickable-search-result":""} ${O?"search-sheet-title":""}"
                                  ...
                                  <span class="search-sheet-subtitle ${this._isClickableSearchResult(C)?"clickable-search-result":""}"
                                        @touchstart=${ue=>this._handleSearchResultTouch(C,ue)}
                                        @click=${()=>this._handleSearchResultClick(C)}>
                                   ${(()=>{const ue=C.media_class==="track",R=this._searchMediaClassFilter==="track"||this._searchMediaClassFilter==="album",Gi=!!this._recentlyPlayedFilterActive,Ln=!!this._upcomingFilterActive,qn=!!this._recommendationsFilterActive;return ue&&C.artist&&C.album?`${C.artist} - ${C.album}`:(R||Gi||Ln||qn)&&C.artist?C.artist:C.media_class?C.media_class.charAt(0).toUpperCase()+C.media_class.slice(1):""})()}
                                 </span>
                                 ${(()=>{const ue=this._isSelectionFlow;return O&&!Ir(C)&&!ue?_`
              ...
              `:this._showGrouping?this._renderGroupingSheet():_`
                ...
                <div class="floating-source-index">
                  ${_e.map((b,O)=>{const G=$e.has(b),tt=this._hoveredSourceLetterIndex;let qe="";if(G&&tt!==null&&tt!==void 0){const C=Math.abs(tt-O);C===0?qe="max":C===1?qe="large":C===2&&(qe="med")}return _`
                      <button
...
`}
            ...
            ${Wa?_`
              <div class="persistent-media-controls" @click=${b=>b.stopPropagation()}>
                ...
                ${(()=>{const b=this._selectedIndex,O=this._getVolumeEntity(b);if(!O)return v;const G=O.startsWith&&O.startsWith("remote."),tt=this.currentVolumeStateObj,qe=Number(tt?.attributes?.volume_level??0),C=G?null:`${Math.round((qe||0)*100)}%`;return this.config.volume_mode==="hidden"?v:_`
