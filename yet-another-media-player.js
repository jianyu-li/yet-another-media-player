        ...
        ${(n||[]).length===0&&!r?_`<div class="search-sheet-empty">${h("common.no_results")}</div>`:(n||[]).map(I=>{const Z=tt(I.media_content_id),K=I.is_browsable&&(I.media_class!=="playlist"||O);return _`
                <div class="search-sheet-result ${M==="card"?"search-result-card":""}">
                  <div class="search-sheet-thumb-container" 
                    ...
                    `}
                    ${M==="card"?li({item:I,onPlay:o,onOptionsToggle:F,upcomingFilterActive:p,isMusicAssistant:Z,massQueueAvailable:O,searchView:"card",onMoveUp:L,onMoveDown:G,onMoveNext:C,onRemove:q}):v}
                  </div>
                  ...
                  ${M!=="card"?li({item:I,onPlay:o,onOptionsToggle:F,upcomingFilterActive:p,isMusicAssistant:Z,massQueueAvailable:O,searchView:"list",isInline:!0,onMoveUp:L,onMoveDown:G,onMoveNext:C,onRemove:q}):v}
                  
                  ${ws({item:I,activeSearchRowMenuId:b,onPlayOption:E,onOptionsToggle:F,searchView:M,isQueueItem:Z&&I.queue_item_id&&p&&O,massQueueAvailable:O,onMoveUp:L,onMoveDown:G,onMoveNext:C,onRemove:q})}
...
{% endif %}</pre>
    ...
    `}_renderGroupingMenuOption(){if(this.entityIds.length<=1)return v;const e=this.entityIds.reduce((n,o,l)=>{const c=this._getGroupingEntityId(l),u=this.hass.states[c];return n+(this._isGroupCapable(u)?1:0)},0),t=this._getGroupingEntityId(this._selectedIndex),a=this.hass.states[t],s=this.currentEntityId,r=this._getGroupKey(s)!==s;return e>1&&this._isGroupCapable(a)&&!r?_`
      ...
      <div class="group-list-scroll">
        ...
        `:f.map(g=>{const b=g.id,k=g.groupId,x=(Array.isArray(s?.attributes?.group_members)?s.attributes.group_members:[]).includes(k),F=this.getChipName(b),E=g.isBusy,B=g.busyLabel,M=this.entityIds.indexOf(b),D=this._getVolumeEntityForGrouping(M)||k,O=this.hass.states[D],L=D?.startsWith&&D.startsWith("remote."),G=Number(O?.attributes?.volume_level||0),C=b===e,q=!C;let I=h(r?C?"card.grouping.master":x?"card.grouping.joined":"card.grouping.available":b===l?"card.grouping.current":"card.grouping.available");return E&&(I=B||"Unavailable"),_`
              ...
              <div style="flex:1.8;display:flex;align-items:center;gap:4px;margin:0 6px; min-width:160px;">
                ${L?_`
                  ...
                  `:_`
                    ...
                    />
                  `}
                <span style="min-width:36px;display:inline-block;text-align:right;">${typeof G=="number"?Math.round(G*100)+"%":"--"}</span>
              </div>
              ${q?_`
        ...
        <div class="entity-options-container entity-options-container-opening">
          <div class="entity-options-sheet${d||f?" chips-mode":""} entity-options-sheet-opening" 
                      ...
                      <div class="chip-row search-filter-chips" id="search-filter-chip-row" style="margin-bottom:12px; justify-content: center; align-items: center;">
                        ...
                        ${y.map(H=>_`
                          <button
                            ...
                            style="
                              width: 72px;
                              background: ${R===H?this._customAccent:"#282828"};
                              opacity: ${R===H?"1":"0.8"};
                              font-weight: ${R===H?"bold":"normal"};
                            "
                            ?selected=${R===H}
            ...
            <div class="entity-options-search-results ${this.config.search_view==="card"?"search-results-card-view":"list-view"}" 
              ...
              ${(()=>{this._searchMediaClassFilter;const y=this._getDisplaySearchResults(),R=Math.max(15,this._searchTotalRows||y.length),H=[...y,...Array.from({length:Math.max(0,R-y.length)},()=>null)];return this._searchAttempted&&y.length===0&&!this._searchLoading?_`<div class="entity-options-search-empty">${h("common.no_results")}</div>`:H.map(N=>N?_`
              ...
              `:this._showGrouping?this._renderGroupingSheet():_`
                ...
                <div class="floating-source-index">
                  ${pe.map((y,R)=>{const H=Ae.has(y),N=this._hoveredSourceLetterIndex;let j="";if(H&&N!==null&&N!==void 0){const Se=Math.abs(N-R);Se===0?j="max":Se===1?j="large":Se===2&&(j="med")}return _`
                      <button
                        class="source-index-letter"
                        ?disabled=${!H}
                        data-scale=${j}
                        @mouseenter=${H?()=>{this._hoveredSourceLetterIndex=R,this.requestUpdate()}:v}
                        @mouseleave=${()=>{this._hoveredSourceLetterIndex=null,this.requestUpdate()}}
                        @click=${H?()=>this._scrollToSourceLetter(y):v}
                      >
                        ${y}
                      </button>
                    `})}
            ...
            ${Ha?_`
              <div class="persistent-media-controls" @click=${y=>y.stopPropagation()}>
                <div class="persistent-controls-artwork">
                  ${(()=>{const y=this.currentPlaybackStateObj,R=this.currentStateObj,H=this._getArtworkUrl(y)||this._getArtworkUrl(R);return H?.url&&this._isValidArtworkUrl(H.url)?_`
                ...
                ${(()=>{const y=this._selectedIndex,R=this._getVolumeEntity(y);if(!R)return v;const H=R.startsWith&&R.startsWith("remote."),N=this.currentVolumeStateObj,j=Number(N?.attributes?.volume_level??0),Se=H?null:`${Math.round((j||0)*100)}%`;return this.config.volume_mode==="hidden"?v:_`
