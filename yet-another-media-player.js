...
`,So=[{mode:"replace",icon:"mdi:playlist-remove",label:h("search.replace")},{mode:"next",icon:"mdi:playlist-play",label:h("search.play_next")},{mode:"replace_next",icon:"mdi:playlist-music",label:h("search.replace_play")},{mode:"add",icon:"mdi:playlist-plus",label:h("search.add_queue")},{mode:"add_to_playlist",icon:"mdi:plus",label:h("search.add_to_playlist")}],Tt=["artist","album","track","playlist","radio","podcast","audiobook"],Ne=(i,{cap:e,floor:t}={})=>{const a=Number(i);if(!Number.isFinite(a)||a<=0)return;let s=a;return typeof t=="number"&&(s=Math.max(t,s)),typeof e=="number"&&(s=Math.min(e,s)),s},ys=3e4;let Mt=null,Ki=0;async function oi(i){const e=Date.now();if(Mt&&e-Ki<ys)return Mt;try{return Mt=(await i.callApi("GET","config/config_entries/entry")).find(t=>t.domain==="music_assistant"&&t.state==="loaded")?.entry_id||null,Ki=e,Mt}catch(t){return console.error("yamp: Failed to resolve Music Assistant config entry",t),Mt=null,Ki=e,null}}let Ft=null,Xi=0;async function bs(i){const e=Date.now();if(Ft&&e-Xi<ys)return Ft;try{return Ft=(await i.callApi("GET","config/config_entries/entry")).find(t=>t.domain==="mass_queue"&&t.state==="loaded")?.entry_id||null,Xi=e,Ft}catch(t){return console.error("yamp: Failed to resolve mass_queue config entry",t),Ft=null,Xi=e,null}}function Dt(i){return i?{title:i.name,media_content_id:i.uri,media_content_type:i.media_type,media_class:i.media_type,thumbnail:i.image,...i.artists&&{artist:i.artists.map(e=>e.name).join(", ")},...i.album&&{album:i.album.name},is_browsable:i.media_type==="artist"||i.media_type==="album"||i.media_type==="playlist"}:null}function li({item:i,onPlay:e,onOptionsToggle:t,upcomingFilterActive:a=!1,isMusicAssistant:s=!1,massQueueAvailable:r=!1,searchView:n="list",isInline:o=!1,onMoveUp:l,onMoveDown:c,onMoveNext:u,onRemove:d}){const p=!!(a&&i.queue_item_id&&s&&r),_=o?"entity-options-search-buttons":n==="card"?"card-overlay-buttons":"search-sheet-buttons",f=o?"entity-options-search-play":n==="card"?"search-sheet-play icon-only":"search-sheet-play",g=o?"entity-options-search-queue":n==="card"?"search-sheet-queue icon-only":"search-sheet-queue";return m`
  ...
  `}function xs({item:i,activeSearchRowMenuId:e,successSearchRowMenuId:t,onPlayOption:a,onOptionsToggle:s,searchView:r="list",isQueueItem:n=!1,onMoveUp:o,onMoveDown:l,onMoveNext:c,onRemove:u}){const d=e!=null&&i.media_content_id!=null&&e===i.media_content_id,p=t===i.media_content_id;return m`
    <div class="search-row-slide-out ${d?"active":""}">
      ...
      `:m`
        <button class="slide-out-button" @click=${()=>a(i,"replace")} title="${h("search.labels.replace")}">
          ${r==="card"?y:m`<ha-icon icon="mdi:playlist-remove"></ha-icon>`}${h("search.labels.replace")}
        </button>
        <button class="slide-out-button" @click=${()=>a(i,"next")} title="${h("search.labels.next")}">
          ${r==="card"?y:m`<ha-icon icon="mdi:playlist-play"></ha-icon>`}${h("search.labels.next")}
        </button>
        <button class="slide-out-button" @click=${()=>a(i,"replace_next")} title="${h("search.labels.replace_next")}">
          ${r==="card"?y:m`<ha-icon icon="mdi:playlist-music"></ha-icon>`}${h("search.labels.replace_next")}
        </button>
        <button class="slide-out-button" @click=${()=>a(i,"add")} title="${h("search.labels.add")}">
          ${r==="card"?y:m`<ha-icon icon="mdi:playlist-plus"></ha-icon>`}${h("search.labels.add")}
        </button>
        ${i.media_class==="track"||i.media_content_type==="track"?m`
          <button class="slide-out-button" @click=${()=>a(i,"add_to_playlist")} title="${h("search.labels.add_to_playlist")}">
            ${r==="card"?y:m`<ha-icon icon="mdi:plus"></ha-icon>`}${h("search.labels.add_to_playlist")}
          </button>
        `:y}
      `}
  ...
  `}function $o({open:i,query:e,onQueryInput:t,onSearch:a,onClose:s,loading:r,results:n,onPlay:o,onQueue:l,error:c,showQueueSuccess:u,matchTheme:d=!1,upcomingFilterActive:p=!1,disableAutofocus:_=!1,activeSearchRowMenuId:f,successSearchRowMenuId:g,onOptionsToggle:b,onPlayOption:k,onResultClick:x,searchView:M="list",searchCardColumns:E=4,massQueueAvailable:G=!1,onMoveUp:U,onMoveDown:O,onMoveNext:z,onRemove:N}){return i?m`
        ...
        ${(n||[]).length===0&&!r?m`<div class="search-sheet-empty">${h("common.no_results")}</div>`:(n||[]).map(C=>{const I=tt(C.media_content_id),R=C.is_browsable&&(C.media_class!=="playlist"||G);return m`
                <div class="search-sheet-result ${M==="card"?"search-result-card":""}">
                  <div class="search-sheet-thumb-container" 
                    ...
                    `}
                    ${M==="card"?li({item:C,onPlay:o,onOptionsToggle:b,upcomingFilterActive:p,isMusicAssistant:I,massQueueAvailable:G,searchView:"card",onMoveUp:U,onMoveDown:O,onMoveNext:z,onRemove:N}):y}
                  </div>
                  <div class="search-sheet-info">
                    ...
                    `:y}
                    ${M==="card"?m`
                      <div class="card-menu-button" @click=${B=>{B.preventDefault(),B.stopPropagation(),b(C)}}>
                        <ha-icon icon="mdi:dots-vertical"></ha-icon>
                      </div>
                    `:y}
                  </div>
                  ${M!=="card"?li({item:C,onPlay:o,onOptionsToggle:b,upcomingFilterActive:p,isMusicAssistant:I,massQueueAvailable:G,searchView:"list",isInline:!0,onMoveUp:U,onMoveDown:O,onMoveNext:z,onRemove:N}):y}
                  
                  ${xs({item:C,activeSearchRowMenuId:f,successSearchRowMenuId:g,onPlayOption:k,onOptionsToggle:b,searchView:M,isQueueItem:I&&C.queue_item_id&&p&&G,onMoveUp:U,onMoveDown:O,onMoveNext:z,onRemove:N})}
                </div>
              `})}
      </div>
    </div>
...
{% endif %}</pre>
    ...
    `}_renderGroupingMenuOption(){if(this.entityIds.length<=1)return y;const e=this.entityIds.reduce((n,o,l)=>{const c=this._getGroupingEntityId(l),u=this.hass.states[c];return n+(this._isGroupCapable(u)?1:0)},0),t=this._getGroupingEntityId(this._selectedIndex),a=this.hass.states[t],s=this.currentEntityId,r=this._getGroupKey(s)!==s;return e>1&&this._isGroupCapable(a)&&!r?m`
      ...
      <div class="group-list-scroll">
        ...
        `:f.map(g=>{const b=g.id,k=g.groupId,x=(Array.isArray(s?.attributes?.group_members)?s.attributes.group_members:[]).includes(k),M=this.getChipName(b),E=g.isBusy,G=g.busyLabel,U=this.entityIds.indexOf(b),O=this._getVolumeEntityForGrouping(U)||k,z=this.hass.states[O],N=O?.startsWith&&O.startsWith("remote."),C=Number(z?.attributes?.volume_level||0),I=b===e,R=!I;let B=h(r?I?"card.grouping.master":x?"card.grouping.joined":"card.grouping.available":b===l?"card.grouping.current":"card.grouping.available");return E&&(B=G||"Unavailable"),m`
                ...
                ${N?m`
                  ...
                  `:m`
                    <input
                      ...
                      step="0.01"
                      .value=${C}
                      @change=${te=>this._onGroupVolumeChange(b,O,te)}
                      title="Volume"
            ...
            <div class="entity-options-search-results ${this.config.search_view==="card"?"search-results-card-view":"list-view"}" 
                               ...
                               <div class="search-sheet-info">
                                <span class="${this._isClickableSearchResult(L)?"clickable-search-result":""} ${this.config.search_view==="card"?"search-sheet-title":""}"
                                 ...
                                 <span class="search-sheet-subtitle">
                                  ${(()=>{const F=this._searchMediaClassFilter==="track"||this._searchMediaClassFilter==="album",Se=!!this._recentlyPlayedFilterActive,Cn=!!this._upcomingFilterActive,Tn=!!this._recommendationsFilterActive;return(F||Se||Cn||Tn)&&L.artist?L.artist:L.media_class?L.media_class.charAt(0).toUpperCase()+L.media_class.slice(1):""})()}
                                </span>
              ...
              `:this._showGrouping?this._renderGroupingSheet():m`
                ...
                <div class="floating-source-index">
                  ${pe.map((v,P)=>{const H=Ae.has(v),L=this._hoveredSourceLetterIndex;let F="";if(H&&L!==null&&L!==void 0){const Se=Math.abs(L-P);Se===0?F="max":Se===1?F="large":Se===2&&(F="med")}return m`
                      <button
...
`}
            ...
            ${Ga?m`
              <div class="persistent-media-controls" @click=${v=>v.stopPropagation()}>
                ...
                ${(()=>{const v=this._selectedIndex,P=this._getVolumeEntity(v);if(!P)return y;const H=P.startsWith&&P.startsWith("remote."),L=this.currentVolumeStateObj,F=Number(L?.attributes?.volume_level??0),Se=H?null:`${Math.round((F||0)*100)}%`;return this.config.volume_mode==="hidden"?y:m`
        ...
        `:y}
          ${this._searchActiveOptionsItem?Io({item:this._searchActiveOptionsItem,onClose:()=>{this._searchActiveOptionsItem=null,this.requestUpdate()},onPlayOption:(v,P)=>this._performSearchOptionAction(v,P)}):y}
