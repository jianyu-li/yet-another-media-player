...
`,To=[{mode:"replace",icon:"mdi:playlist-remove",label:h("search.replace")},{mode:"next",icon:"mdi:playlist-play",label:h("search.play_next")},{mode:"replace_next",icon:"mdi:playlist-music",label:h("search.replace_play")},{mode:"add",icon:"mdi:playlist-plus",label:h("search.add_queue")},{mode:"add_to_playlist",icon:"mdi:plus",label:h("search.add_to_playlist")}],jt=["artist","album","track","playlist","radio","podcast","audiobook"];function xs(i){return i&&(i.media_class==="track"||i.media_content_type==="track")}const Ne=(i,{cap:e,floor:t}={})=>{const a=Number(i);if(!Number.isFinite(a)||a<=0)return;let s=a;return typeof t=="number"&&(s=Math.max(t,s)),typeof e=="number"&&(s=Math.min(e,s)),s},ws=3e4;let Pt=null,Zi=0;async function li(i){const e=Date.now();if(Pt&&e-Zi<ws)return Pt;try{return Pt=(await i.callApi("GET","config/config_entries/entry")).find(t=>t.domain==="music_assistant"&&t.state==="loaded")?.entry_id||null,Zi=e,Pt}catch(t){return console.error("yamp: Failed to resolve Music Assistant config entry",t),Pt=null,Zi=e,null}}let Dt=null,Ki=0;async function Xi(i){const e=Date.now();if(Dt&&e-Ki<ws)return Dt;try{return Dt=(await i.callApi("GET","config/config_entries/entry")).find(t=>t.domain==="mass_queue"&&t.state==="loaded")?.entry_id||null,Ki=e,Dt}catch(t){return console.error("yamp: Failed to resolve mass_queue config entry",t),Dt=null,Ki=e,null}}function dt(i){return i?{title:i.name,media_content_id:i.uri,media_content_type:i.media_type,media_class:i.media_type,item_id:i.item_id,thumbnail:i.image,...i.artists&&{artist:i.artists.map(e=>e.name).join(", ")},...i.album&&{album:i.album.name,album_uri:i.album.uri},is_browsable:i.media_type==="artist"||i.media_type==="album"||i.media_type==="playlist"||i.media_type==="track",is_editable:i.is_editable===!0}:null}function ks({item:i,onPlay:e,onOptionsToggle:t,upcomingFilterActive:a=!1,isMusicAssistant:s=!1,massQueueAvailable:r=!1,searchView:n="list",isInline:o=!1,onMoveUp:l,onMoveDown:c,onMoveNext:u,onRemove:d}){const p=!!(a&&i.queue_item_id&&s&&r),m=o?"entity-options-search-buttons":n==="card"?"card-overlay-buttons":"search-sheet-buttons",f=o?"entity-options-search-play":n==="card"?"search-sheet-play icon-only":"search-sheet-play",g=o?"entity-options-search-queue":n==="card"?"search-sheet-queue icon-only":"search-sheet-queue";return _`
    <div class="${m}">
      ...
      ${!p&&i.media_class!=="radio"&&i.media_content_type!=="radio"?_`
        <button class="${g}" 
        ...
        <div class="entity-options-container entity-options-container-opening">
          <div class="entity-options-sheet${d||f?" chips-mode":""} entity-options-sheet-opening" 
            ...
            <div class="entity-options-search-results ${this.config.search_view==="card"?"search-results-card-view":"list-view"}" 
              ...
              ${(()=>{this._searchMediaClassFilter;const b=this._getDisplaySearchResults(),q=Math.max(15,this._searchTotalRows||b.length),Q=[...b,...Array.from({length:Math.max(0,q-b.length)},()=>null)];return this._searchAttempted&&b.length===0&&!this._searchLoading?_`<div class="entity-options-search-empty">${h("common.no_results")}</div>`:Q.map(C=>C?_`
                            <!-- EXISTING non‑placeholder row markup -->
                            <div class="entity-options-search-result ${this.config.search_view==="card"?"search-result-card":""} ${C._justMoved?"just-moved":""} ${C.media_content_id!=null&&this._activeSearchRowMenuId===C.media_content_id?"menu-active":""}">
                               <div class="search-sheet-thumb-container"
                                    data-clickable="${this.config.search_view==="card"}"
                                    @click=${this.config.search_view==="card"?()=>this._playMediaFromSearch(C):null}>
                                ...
                                `}
                                ${this.config.search_view==="card"?ks({item:C,onPlay:M=>this._playMediaFromSearch(M),onOptionsToggle:M=>{this._activeSearchRowMenuId=M?.media_content_id||null,this.requestUpdate()},upcomingFilterActive:!!this._upcomingFilterActive,isMusicAssistant:this._isMusicAssistantEntity(),massQueueAvailable:this._massQueueAvailable,searchView:"card"}):v}
                               </div>
                               <div class="search-sheet-info">
                                <span class="${this._isClickableSearchResult(C)?"clickable-search-result":""} ${this.config.search_view==="card"?"search-sheet-title":""}"
                                      @touchstart=${M=>this._handleSearchResultTouch(C,M)}
                                 ...
                                 <span class="search-sheet-subtitle ${this._isClickableSearchResult(C)?"clickable-search-result":""}"
                                       @touchstart=${M=>this._handleSearchResultTouch(C,M)}
                                       @click=${()=>this._handleSearchResultClick(C)}>
                                  ${(()=>{const M=C.media_class==="track",$e=this._searchMediaClassFilter==="track"||this._searchMediaClassFilter==="album",Fn=!!this._recentlyPlayedFilterActive,jn=!!this._upcomingFilterActive,Pn=!!this._recommendationsFilterActive;return M&&C.artist&&C.album?`${C.artist} - ${C.album}`:($e||Fn||jn||Pn)&&C.artist?C.artist:C.media_class?C.media_class.charAt(0).toUpperCase()+C.media_class.slice(1):""})()}
                                </span>
                                ${this.config.search_view==="card"&&C.media_class!=="radio"&&C.media_content_type!=="radio"?_`
                                  <div class="card-menu-button" @click=${M=>{M.preventDefault(),M.stopPropagation(),this._activeSearchRowMenuId=C.media_content_id,this.requestUpdate()}}>
                                    <ha-icon icon="mdi:dots-vertical"></ha-icon>
                                  </div>
                                `:v}
                              </div>
              ...
              `:this._showGrouping?this._renderGroupingSheet():_`
                ...
                <div class="floating-source-index">
                  ${pe.map((b,q)=>{const Q=Se.has(b),C=this._hoveredSourceLetterIndex;let M="";if(Q&&C!==null&&C!==void 0){const $e=Math.abs(C-q);$e===0?M="max":$e===1?M="large":$e===2&&(M="med")}return _`
                      <button
...
`}
            ...
            ${Ga?_`
              <div class="persistent-media-controls" @click=${b=>b.stopPropagation()}>
                ...
                ${(()=>{const b=this._selectedIndex,q=this._getVolumeEntity(b);if(!q)return v;const Q=q.startsWith&&q.startsWith("remote."),C=this.currentVolumeStateObj,M=Number(C?.attributes?.volume_level??0),$e=Q?null:`${Math.round((M||0)*100)}%`;return this.config.volume_mode==="hidden"?v:_`
